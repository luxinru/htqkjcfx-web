    ...
    <div class="chart-legend">
      <div class="legend-item" v-for="(item, index) in pieCharArr" :key="index">
        <div class="item-left">
          ...
          <div class="xmsl-box">
            <span class="xmsl">{{ $money100m(item.xmsl) }} </span
            ><span class="unit">亿元</span>
          </div>
          <div class="ndjiz">年度计划值：{{ $money100m(item.ndjhz) }}亿元</div>
        </div>
        <div class="item-right">
          <div class="legend-text">年度计划完成率</div>
          <el-progress
            type="circle"
            :percentage="Number(item.xmzb)"
            :text-color="'#00f886'"
            :stroke-width="5"
            :width="80"
            :define-back-color="'#153e75'"
            :color="'#9afffe'"
          ></el-progress>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async getData() {
      const result = await api.getZdxmtzjefbtj(this.form);
      // 更新第一个元素的属性
